...
<div class="signup-form">
    <s:form action="edit-item" method="post" enctype="multipart/form-data">
        <h2>Edit Item</h2>
        <p class="hint-text">Edit item in database</p>
        <div class="form-group">
            <input type="text" class="form-control" name="ino" value='<%= request.getParameter("ino") %>'
                   placeholder="Item ID" required="required" readonly="true">
        </div>
                ...
                <div class="col">
                    <select name="itno" id="selectItemType" onchange="changeItemType()">
                        <c:forEach items="${allItemTypes}" var="itemType">
                            <c:choose>
                                <c:when test="${itnoVar==itemType.itno}">
                                    <option value="${itemType.itno}" selected>${itemType.itname}</option>
                                </c:when>
                                <c:otherwise>
        ...
        <div class="form-group">
            <input type="text" class="form-control" name="iname" value='<%= request.getParameter("iname") %>'
                   placeholder="Item name" required="required">
        </div>
        <div class="form-group">
            <s:fielderror cssStyle="color:red" fieldName="unitPriceField"/>
            <input type="number" class="form-control" name="unitPrice" value='<%= request.getParameter("unitPrice") %>'
                   placeholder="Unit price" required="required">
        </div>
        <div class="form-group">
            <s:fielderror cssStyle="color:red" fieldName="dateImportField"/>
            <input type="date" class="form-control" name="dateImport" value='<%= request.getParameter("dateImport") %>'
                   placeholder="Date import" required="required">
        </div>
        <div class="form-group">
            <s:fielderror cssStyle="color:red" fieldName="dateExpiredField"/>
            <input type="date" class="form-control" name="dateExpired"
                   value='<%= request.getParameter("dateExpired") %>' placeholder="Date expired" value="${null}">
        </div>
        <div class="form-group">
            <s:fielderror cssStyle="color:red" fieldName="avaiAmountField"/>
            <input type="number" class="form-control" name="avaiAmount"
                   value='<%= request.getParameter("avaiAmount") %>' placeholder="Available amount"
                   required="required"/>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <button type="submit" class="btn btn-success btn-lg btn-block">Edit Item</button>
        </div>
    </s:form>
    <div class="form-group">
